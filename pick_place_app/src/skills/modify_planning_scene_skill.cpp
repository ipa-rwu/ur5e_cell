...
static const rclcpp::Logger LOGGER =
    rclcpp::get_logger("modify_planning_scene_skill");
...
moveit_msgs::msg::CollisionObject ModifyPlanningSceneSkill::createBox(
...
void ModifyPlanningSceneSkill::addObject(
    moveit_msgs::msg::CollisionObject& object, bool invert) {
  if (!invert) {
...
void ModifyPlanningSceneSkill::allowCollisions(const Names& first,
                                               const Names& second,
                                               bool allow) {
...
void ModifyPlanningSceneSkill::allowCollisions(
    planning_scene::PlanningScenePtr& scene, const CollisionMatrixPairs& pairs,
    bool invert) {
  collision_detection::AllowedCollisionMatrix& acm =
      scene->getAllowedCollisionMatrixNonConst();
  bool allow = invert ? !pairs.allow : pairs.allow;
  if (pairs.second.empty()) {
    for (const auto& name : pairs.first) acm.setEntry(name, allow);
  } else
    acm.setEntry(pairs.first, pairs.second, allow);
}

void ModifyPlanningSceneSkill::attachObject(
    const std::string& group, moveit_msgs::msg::CollisionObject& object,
