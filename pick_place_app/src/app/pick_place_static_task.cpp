...
void PickPlaceStaticTask::executeTask()
{
  ...
  if (step_success)
  {
    ...
    pre_pick_point_msg.point.z = param_pick_place_static_.object_pose.position.z +
  ...
  if (step_success)
  {
    ...
    step_success =
        comp_path_skill->computeRelative(param_pick_place_static_.arm_group_name, direction,
                                         robot_trajectories, param_pick_place_static_.ik_frame);
    if (step_success)
    {
      for (const auto& traj : robot_trajectories)
      {
        step_success =
            exec_traj_skill->execute_trajectory(param_pick_place_static_.arm_group_name, traj);
      }
    }
    rclcpp::sleep_for(1s);
  }
  ...
  modify_planning_scene_skill->attachObject(param_pick_place_static_.arm_group_name, object_box,
                                            attach_link, false);
  ...
  if (step_success)
  {
    ...
    pre_place_point_msg.header.frame_id = param_pick_place_static_.place_frame_id;
    pre_place_point_msg.point = param_pick_place_static_.place_pose.position;
    pre_place_point_msg.point.x = param_pick_place_static_.place_pose.position.x +
                                  param_pick_place_static_.pre_place_offset.at(0);
    pre_place_point_msg.point.y = param_pick_place_static_.place_pose.position.y +
                                  param_pick_place_static_.pre_place_offset.at(1);
    pre_place_point_msg.point.z = param_pick_place_static_.place_pose.position.z +
                                  param_pick_place_static_.pre_place_offset.at(2);
    step_success =
        comp_path_skill->computePath(param_pick_place_static_.arm_group_name, pre_place_point_msg,
