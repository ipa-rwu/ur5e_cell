           ...
           std::shared_ptr<ur_msgs::srv::SetIO::Response> response)
{
  ...
  RCLCPP_INFO(rclcpp::get_logger("mock_io_server"),
              "Setting digital output '%d' to state: '%1.0f'.", request->pin, request->state);
  return true;
}
...
int main(int argc, char** argv)
{
  ...
  RCLCPP_INFO(rclcpp::get_logger("mock_io_server"), "mocking IO server.");
